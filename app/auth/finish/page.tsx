...
function FinishEmailLinkContent() {
  const router = useRouter();
  const { finishEmailLinkSignIn, user, ready } = useAuth();
  ...
  useEffect(() => {
    const handleSignIn = async () => {
      ...
      try {
        await finishEmailLinkSignIn(String(seed.email), String(url));
        clearEmailLinkSeed();
