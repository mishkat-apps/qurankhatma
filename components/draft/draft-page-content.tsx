...
export function DraftPageContent({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    setStore(nextStore);
    const nextDraft = nextStore.getDraft(id);
    if (nextDraft) {
      setDraft(nextDraft);
  ...
  const promote = useMemo(
    () => async () => {
      ...
      try {
        ...
        router.replace(`/khatma?id=${promoted.id}`);
      } catch (error) {
  ...
  if (!draft) {
    return (
      <main className="page-shell pb-0">
        <SiteHeader />
        <div className="mx-auto max-w-3xl px-4 md:px-6 pt-12">
          <Panel>
            <h1 className="font-[var(--font-heading)] text-5xl">Draft not found</h1>
            <p className="mt-3 text-muted">This local draft exists only in the browser where it was created.</p>
          </Panel>
        </div>
        <SiteFooter />
      </main>
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 pt-12 md:px-6 md:pt-20">
        <KhatmaView
          ...
          onUndo={(juzNumber) => {
            const nextJuz = draft.juz.map((record) =>
              record.juzNumber === juzNumber
                ? ({ ...record, state: 'claimed', completedAt: null } as JuzRecord)
                : record,
            );
            persistDraft({ ...draft, juz: nextJuz });
          }}
